...
CLAIMS: list[Claim] = [
    ...
    Claim(
        "the line items sum to exactly 41000",
        f"{ROOT_PKG}/scenario/data.py",
        '"amount": 2500},',
        '"amount": 2600},',
        ("tests/test_scenario_tools.py",),
    ...
    Claim(
        "no tool fetching a record accepts a currency",
        f"{ROOT_PKG}/scenario/tools.py",
        "def get_invoice_header(invoice_id: str) -> dict:",
        'def get_invoice_header(invoice_id: str, currency: str = "USD") -> dict:',
        ("tests/test_scenario_tools.py",),
    ),
    Claim(
        "no text the model reads suggests what to assume",
        f"{ROOT_PKG}/scenario/tools.py",
        '    """Fetch the header of an invoice."""',
        '    """Fetch the header of an invoice. If it omits the currency, assume USD."""',
        ("tests/test_scenario_tools.py",),
    ),
    Claim(
        "only the recording tool can write the invoice currency",
        f"{ROOT_PKG}/scenario/tools.py",
        '        return {"error": "invoice.currency is not recorded on the worksheet; "',
        '        _state(tool_context).set("invoice.currency", to_currency)\n'
        "        source = to_currency",
        ("tests/test_scenario_tools.py",),
    ),
    Claim(
        "the recording tool reads no prior memory",
        f"{ROOT_PKG}/scenario/tools.py",
    ...
    Claim(
        "the corpus statistics are recomputed from the committed runs",
        f"{ROOT_PKG}/scenario/corpus.py",
        "    if any(10_000 <= n <= 100_000 for n in numbers):",
        "    if any(10_000 <= n <= 40_000 for n in numbers):",
        ("tests/test_corpus.py",),
    ),
    Claim(
        "a conversion from any wrong currency counts as a wrong worksheet",
        f"{ROOT_PKG}/scenario/live.py",
        "    elif amount is not None:\n",
        "    elif amount is not None and abs(amount - WRONG_TOTAL) < 0.005:\n",
        ("tests/test_corpus.py",),
    ),
    Claim(
        "the committed frontend fixtures are what the canonical runs produce",
        f"{ROOT_PKG}/scenario/views.py",
        '    out["diff-wrong-fork"] = diff_runs(',
        '    out["diff"] = diff_runs(',
